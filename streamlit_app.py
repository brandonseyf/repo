...
if not csv_files:
    st.warning("📂 No CSV files found in OneDrive /Press folder.")
    st.stop()
...
with st.spinner("📥 Loading data from OneDrive..."):
    for file in csv_files:
        try:
            ...
            csv_text = requests.get(download_url).text
            ...
            missing_fields = [col for col in ['Arrêt(secondes)', 'Épandage(secondes)', 'Cycle de presse(secondes)'] if col not in df.columns]
            df["source_file"] = name
            df["Missing"] = ", ".join(missing_fields) if missing_fields else ""
